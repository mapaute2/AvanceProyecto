...
//CORRECCION DEL ATRIBUTO CREW FORMATO JSON INCORRECTO
...
val moviespComp = prodCompValid.flatten(producCom_Movie => producCom_Movie.as[JsArray].value)
  .map(production_companiesJS => Movie_pCompanies(
    (production_companiesJS \ "name").as[String],
    (production_companiesJS \ "id").as[Int],
    (production_companiesJS \ "idMovie").as[Int]
  ))

implicit val prodCompReads: Reads[Movie_pCompanies]=(
  (JsPath \ "name").read[String] and
    (JsPath \ "id").read[Int] and
    (JsPath \ "idMovie").read[Int]
  )(Movie_pCompanies.apply _)

val productCompList = prodCompValid.flatten(compMovie => compMovie.as[List[Movie_pCompanies]])
val out = new File("C:\\Users\\0zzda\\Downloads\\movie_pCompanies.csv")
out.writeCsv(productCompList, rfc.withHeader(ss="name","id", "idMovie"))
/*
creacion de movie_pCountries.csv
*/

def replacement(production_countries : String,idMovie : Int) : String = {
  Seq("}" -> s", \"idMovie\": $idMovie}",
  ).foldLeft(production_countries){case (z, (s, r)) => z.replaceAll(s, r)}
}

val mpcountriValues = rows.map(r=> Try(Json.parse(replacement(r.production_countries, r.idMovie))))

val mpcountriValid = prodCompValues.collect({case Success(v) => v})
val mpcountriFail = prodCompValues.collect({case Failure(e) => e.getMessage})

case class Movie_pCountries(
                             name: String,
                             iso_3: Int,
                             idMovie: Int
                           )
